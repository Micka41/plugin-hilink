...
class CustomHttpClient
{
	...
	*/
	public function postXml($url, $xmlString, $try = 0)
	{
		...
		if(!$result)
		{
			...
			{
				sleep(5);
				$retour = $this->postXml($url, $xmlString, 1);
			} else {
				throw new \Exception("Erreur cURL postXml <" . $url . "> : " . $error);	
			}
		}
		else
		{
	...
	*/
	public function get($url, $try = 0)
	{
		...
		if(!$result)
		{
			...
			{
				sleep(5);
				$retour = $this->get($url, 1);
			} else {
				throw new \Exception("Erreur cURL get <" . $url . "> : " . $error);	
			}
		}
		else
		{
